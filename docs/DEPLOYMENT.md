...
- отдельный Cloudflare Worker из `scheduler/` с Cron Trigger `* * * * *`;
- Telegram-бот;
...
Планировщик разворачивается из `scheduler/wrangler.toml`. Секрет `CRON_SECRET` задаётся через `wrangler secret put CRON_SECRET` и должен совпадать с секретом backend; в репозиторий его не добавляют. GitHub Actions оставлен только для ручной аварийной проверки, чтобы два автоматических расписания не создавали дублирующие запросы.

Cron Trigger `* * * * *` хранится в конфигурации Worker. Все cron-выражения Cloudflare выполняются в UTC, но день рождения вычисляется в сохранённом часовом поясе.
